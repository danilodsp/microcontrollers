...
void main(void)
{
	...
	P1SEL |= TXD;
	P1DIR |= TXD;
	P1DIR |= (BIT3)|(BIT4)|(BIT5); // LEDs
	P2DIR |= (BIT0)|(BIT1)|(BIT2); // LEDs
	...
	while(1)
	{
		if (hasReceived)		// If the device has recieved a value
		{
			...
			while(RXByte==0x0D);
			while(cont<3){
				if((hasReceived)&&(cont==1)){
					Valor = Valor + (RXByte|0x0F);
				}
				else if((hasReceived)&(cont==2)){
					Valor = Valor + ((RXByte|0x0F)*10);
				}
				else if((hasReceived)&(cont==3)){
					Valor = Valor + ((RXByte|0x0F)*100);
				}
				cont++;					
			}
			cont = 0;
